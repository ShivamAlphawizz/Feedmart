...
class StatePayment extends State<Payment> with TickerProviderStateMixin {
  ...
  List<String> paymentIconList = [
    Platform.isIOS ?
    'assets/images/applepay.svg':'assets/images/gpay.svg',
                    ...
                    child: Column(
                      children: [
                        ...
                        SimBtn(
                          ...
                          onBtnSelected: () {
                            Navigator.pop(context, Upi.toString());
                          },
                        ),
                      ],
                    ),
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (!error) {
            ...
            // Upi = data['system_settings'][0]["admin_upi_id"];
            // print("NEWWWWWWWWWWWWWWWWWW=============>>>>>${Upi}");
  ...
  var Upi;
  Future<void> _getdateTime1() async {
    _isNetworkAvail = await isNetworkAvailable();
    if (_isNetworkAvail) {
      timeSlotList.clear();
      try {
        var parameter = {TYPE: ALL, USER_ID: CUR_USERID};
        Response response =
            await post(getSettingApi, body: parameter, headers: headers)
                .timeout(Duration(seconds: timeOut));
